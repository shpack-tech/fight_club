...
/* The coffee_ class extends Game.Object and Game.Animation. */
Game.coffee_ = function (x, y) {
	Game.Object.call(this, x, y, 7, 14);
	Game.Animator.call(this, Game.coffee_.prototype.frame_sets['twirl'], 15);

	this.frame_index = Math.floor(Math.random() * 2);

	/* base_x and base_y are the point around which the coffee_ revolves. position_x
  and y are used to track the vector facing away from the base point to give the coffee_
  the floating effect. */
	this.base_x = x;
	this.base_y = y;
...
Object.assign(Game.coffee_.prototype, Game.Animator.prototype);
Object.assign(Game.coffee_.prototype, Game.Object.prototype);
Game.coffee_.prototype.constructor = Game.coffee_;
...
Game.Door = function (door) {
	...
	this.destination_level = door.destination_level;
};
Game.Door.prototype = {};
...
Game.World = function (friction = 0.85, gravity = 2) {
	...
	this.level_id = localStorage.getItem('level') || '0';

	this.coffee = [];
	this.coffee__count = 0;
...
Game.World.prototype = {
	...
	setup: function (level) {
		...
		for (let index = level.coffee.length - 1; index > -1; --index) {
			let coffee_ = level.coffee[index];
			this.coffee[index] = new Game.coffee_(coffee_[0] * this.tile_set.tile_size + 5, coffee_[1] * this.tile_set.tile_size - 2);
		}
		...
		for (let index = level.grass.length - 1; index > -1; --index) {
			let grass = level.grass[index];
		...
		if (this.door) {
			...
			this.door = undefined; // Make sure to reset this.door so we don't trigger a level load.
		}
	},
	...
	update: function () {
		...
		for (let index = this.coffee.length - 1; index > -1; --index) {
			let coffee_ = this.coffee[index];

			coffee_.updatePosition();
			coffee_.animate();

			if (coffee_.collideObject(this.player)) {
				this.coffee.splice(this.coffee.indexOf(coffee_), 1);
				this.coffee__count++;
				let cup = +localStorage.getItem('coffee');
				cup++;
				localStorage.setItem('coffee', cup);
			}
		}
